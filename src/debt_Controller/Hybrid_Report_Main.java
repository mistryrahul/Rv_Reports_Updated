...
public class Hybrid_Report_Main 
{
...
//			    String Fund_Type="Hybrid : Equity Oriented";
			    String Fund_Type="Hybrid : Arbitrage";
...
//						    				    System.out.println("Date-->"+dt);
				    	 ...
				    	 ret_24_mnths = calculate_return(crsg,-12, ssn);
				    	 ans_1=0;
				    	 ans_2=0;
				    	 ans_3=0;
				    	 fin_res=0;
				    	 if(!Double.toString(ret_24_mnths).equals("-99.99"))
				    	 {
//				    		 System.out.println("36 Months Return-->>"+ret_36_mnths);
				    		 
				    		 ans_1= ((ret_24_mnths/100) +1);
				    		 ans_2= (0.5);
...
//				    		 System.out.println("Result--->>>"+fin_res);
				    		 ...
				    		 ob1.setReturn_24_months(fin_res);
				    	 }
...
//											             
											             for(Hybrid_Report_Model bb : drm)
											             {
											            	 if(bb.getAum()<100)
											            	 {
											            		 bb.setStar("Unrated");
...
//											             Query qry_1 = ssn.createQuery("from Debt_Report_1");
											             ArrayList<Hybrid_Report_Model> drm1 = (ArrayList<Hybrid_Report_Model>) ssn.createQuery("from Hybrid_Report_Model where Fund_Type='"+Fund_Type+"'").list();
											             for(Hybrid_Report_Model ob11 : drm1)
											             {
											            	 double comp_score = ((ob11.getReturn_12_months()* 0.2) + (ob11.getReturn_36_months()*0.5) + (ob11.getReturn_24_months()*0.3));
...
//			              ArrayList<Hybrid_Report_Model> mn_lst_totl = (ArrayList<Hybrid_Report_Model>) ssn.createQuery("from Hybrid_Report_Model where star is null").list();
//	                     
//	                       for(Hybrid_Report_Model ob : mn_lst_totl)
//	                       {
	  ...
	  static double calculate_return(Credit_rating_sum_groups crsg_obj,int months ,Session ssn) throws ParseException
		{
			...
			ArrayList<nav_hist> nav_hst_lst_mn = get_list_of_dates_db(crsg_obj.getInv_date(),crsg_obj.getScheme_code());
			 ...
			 double nav_ret=0;
			  ...
			  java.util.Date d_x = nav_hst_lst_mn.get(0).getKey().getNavdate();
...
//	        ssn.beginTransaction();
	        ...
	        ArrayList<Hybrid_Report_Model> mn_lst_rank_wise = (ArrayList<Hybrid_Report_Model>) ssn.createQuery("from Hybrid_Report_Model where star is null and Fund_Type='"+Fund_Type+"' order by composite_score desc").list();
...
//	       System.out.println("5->"+top_grp_5);
	        ...
	        for(Hybrid_Report_Model ob : mn_lst_rank_wise)
	        {
